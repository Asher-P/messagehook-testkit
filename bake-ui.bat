...
REM so `dotnet run --project playbook\MessageHook.Playbook.Service` serves the UI + API same-origin (no Vite dev server).
setlocal
set "ROOT=%~dp0"
set "UI=%ROOT%playbook\messagehook-ui"
set "WWWROOT=%ROOT%playbook\MessageHook.Playbook.Service\wwwroot"
...
echo   dotnet run --project playbook\MessageHook.Playbook.Service
exit /b 0
